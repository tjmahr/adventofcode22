...
#'
#' @param x some data
#' @return For Part One, `f01a_find_max_calories(x)` returns the total calories
#'   for the elf with maximum calories. For Part Two,
#'   `f01b_find_top_3_calories_sum(x)` returns the total calories of the top 3
#'   calorie-dense elves.
#' @export
#' @examples
#' f01a_find_max_calories(example_data_01())
#' f01b_find_top_3_calories_sum(example_data_01())
f01a_find_max_calories <- function(x) {
  x |>
    f01_helper() |>
    lapply(sum) |>
    unlist() |>
...
#' @rdname day01
#' @export
f01b_find_top_3_calories_sum <- function(x) {
  x |>
    f01_helper() |>
    lapply(sum) |>
    unlist() |>
