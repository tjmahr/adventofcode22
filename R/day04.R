...
#'
#' @param x some data
#' @return For Part One, `f04a_count_contained_ranges(x)` returns .... For Part Two,
#'   `f04b_count_overlaps(x)` returns ....
#' @export
#' @examples
#' f04a_count_contained_ranges(example_data_04())
#' f04b_count_overlaps(example_data_04())
f04a_count_contained_ranges <- function(x) {
  data <- f04_helper(x)
  x_in_y <- data$y1 <= data$x1 & data$x2 <= data$y2
  y_in_x <- data$x1 <= data$y1 & data$y2 <= data$x2
  sum(x_in_y | y_in_x)
}
...
#' @rdname day04
#' @export
f04b_count_overlaps <- function(x) {
  data <- f04_helper(x)
  x1_in_y <- data$y1 <= data$x1 & data$x1 <= data$y2
  x2_in_y <- data$y1 <= data$x2 & data$x2 <= data$y2
  y1_in_x <- data$x1 <= data$y1 & data$y1 <= data$x2
  y2_in_x <- data$x1 <= data$y2 & data$y2 <= data$x2
  sum(x1_in_y | x2_in_y | y1_in_x | y2_in_x)
}
...
f04_helper <- function(x) {
  data <- x |>
    strsplit(",|-") |>
    # Didn't have this at first 😖
    lapply(as.numeric) |>
    lapply(matrix, nrow = 1) |>
    lapply(as.data.frame) |>
    do.call(rbind, args = _) |>
    stats::setNames(c("x1", "x2", "y1", "y2"))
  data$id <- seq_along(x)
  data
}
