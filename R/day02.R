...
#'
#' @param x some data
#' @return For Part One, `f02a_find_total_points(x)` returns the total score.
#'   For Part Two, `f02b_find_total_points(x)` returns the total score.
#' @export
#' @examples
#' f02a_find_total_points(example_data_02())
#' f02b_find_total_points(example_data_02())
f02a_find_total_points <- function(x) {
  f02_find_winner <- function(pair) {
  ...
  picks <- strategy[f02_helper(x)] |>
    matrix(ncol = 2, byrow = TRUE)

  results <- round_points[apply(picks, 1, f02_find_winner)] +
    shape_points[picks[, 2]]
...
#' @rdname day02
#' @export
f02b_find_total_points <- function(x) {
  f02_find_play <- function(pair) {
    # nested lists are trees
    rules <- list(
      rock = list(
        draw = "rock",
  ...
  results <- shape_points[picks |> apply(1, f02_find_play)] +
    round_points[picks[, 2]]
