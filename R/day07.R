...
f07_helper <- function(x) {
  x <- example_data_07()
  x
  file_system <- function(x) {
    is_command <- function(x) {
      substr(x, 1, 1) == "$"
    }
    is_dir_call <- function(x) {
      substr(x, 1, 4) == "dir "
    }
    is_file_call <- function(x) {
      !is_dir_call(x) & !is_command(x)
    }
    get_command <- function(x) {
      strsplit(l, " ") |> unlist() |> getElement(2)
    }
    ...
    current_dir <- ""
    pwd <- function(...) {
      paste0(c(current_dir, ...), collapse = "/")
    }
    cd <- function(l) {
      dest <- substr(l, 6, nchar(l))
      if (dest == "/") {
        current_dir <<- c("root")
      } else if (dest == "..") {
        current_dir <<- current_dir[-length(current_dir)]
      } else {
        current_dir <<- c(current_dir, dest)
      }
      current_dir
    }
    ...
    ls <- function(l) {
      if (is_dir_call(l)) {
        fs <<- c(fs, pwd(substr(l, 5, nchar(l))))
      }
      if (is_file_call(l)) {
        parts <- strsplit(l, " ") |> unlist()
        file <- parts[1] |>
          as.numeric() |>
          setNames(pwd(parts[2]))
        fs <<- c(fs, pwd(parts[2]))
        files <<- c(files, file)
      }
      l
