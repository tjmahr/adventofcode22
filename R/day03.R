...
#'
#' @param x some data
#' @return For Part One, `f03a_sum_priorities(x)` returns .... For Part Two,
#'   `f03b_sum_priorities(x)` returns ....
#' @export
#' @examples
#' f03a_sum_priorities(example_data_03())
#' f03b_sum_priorities(example_data_03())
f03a_sum_priorities <- function(x) {
      ...
      lapply(
        function(x) {
          a <- substr(x, 1, nchar(x) / 2) |>
            strsplit("") |>
            unlist()
          b <- substr(x, 1 + nchar(x) / 2, nchar(x))|>
            strsplit("") |>
            unlist()
          intersect(a, b)
        }
...
#' @rdname day03
#' @export
f03b_sum_priorities <- function(x) {
  find_intersections <- function(x) {
    trios <- (seq_along(x) - 1) %/% 3
    x |>
      split(trios) |>
      lapply(strsplit, "") |>
